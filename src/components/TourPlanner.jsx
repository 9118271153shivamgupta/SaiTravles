...
const TourPlanner = ({ activeCategoryFilter = null, showViewMoreButton = true, onViewCategory, onViewDetails, onBookNow }) => {
  ...
  // FIX: Hardcoded object ko hata kar dynamic ref compile kiya hai
  // Isse 'nepal' ya koi bhi nayi category automatic register ho jayegi
  const sectionRefs = useRef({});

  const handleTabScroll = (sectionId) => {
    if (activeCategoryFilter) {
      // Agar direct filter page par hain toh parent page par redirect ya state filter chalega
      onViewCategory(sectionId);
    } else {
      // Home page par hain toh usi section par smooth scroll target dynamic calculate karega
        ...
        <div className="space-y-24">
          {activeCategories.map((category) => {
            ...
            return (
              <div 
                key={category.id} 
                // FIX: DOM element ko dynamic ref key assign ki hai
