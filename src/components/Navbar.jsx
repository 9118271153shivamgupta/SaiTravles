      ...
      <nav className="bg-white sticky top-0 z-50 shadow-md transition-all duration-300 border-b-2 border-amber-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-24 items-center">
              ...
              {navLinks.map((link, index) => {
                ...
                return (
                  <Link
                    key={index}
                    to={link.path}
                    className={`text-sm font-extrabold tracking-wide transition-all duration-200 relative py-2 uppercase
                      ${isActive ? 'text-sky-600 font-black' : 'text-sky-950 hover:text-amber-500'}`}
                  >
                    {link.name}
                    {isActive && (
            ...
            {/* Desktop Animated Premium Button (Logo Blue-Orange Theme) */}
            <div className="hidden lg:block">
              <button 
                onClick={() => setIsFormOpen(true)}
                className="relative overflow-hidden group px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-black tracking-wider shadow-md transition-all duration-300 hover:bg-amber-500 hover:scale-105 active:scale-95 cursor-pointer flex items-center gap-2 uppercase"
              >
                {/* Glow Element Effect */}
        ...
        {/* Mobile Responsive Menu (Theme Balanced according to logo.png) */}
        {isOpen && (
...
//                 className="h-18 w-28 object-cover rounded-full   shadow-inner"
